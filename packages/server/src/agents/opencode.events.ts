                ...
                if (!hasError) {
                    if (agentType === 'code_review') {
                        ...
                        try {
                            ...
                            // The Code Review agent can now handle local worktrees directly.
                            // Even if PR creation fails, we can mark as "done" and move the ticket
                            // so the local review process can start.
                            updateSessionComment(`⚠️ **PR Creation / Push Failed**\n\nThe agent could not complete the PR lifecycle:\n\`\`\`\n${error.message}\n\`\`\`\n\nHowever, your changes are saved locally. Moving ticket to Code Review for local analysis.\n\nYou can review changes manually at: ${worktreePath}`, 'status');
